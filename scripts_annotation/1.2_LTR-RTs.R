...
# ---- Package setup with user library ----
user_lib <- Sys.getenv("R_LIBS_USER", unset = file.path("~", "R", "x86_64-pc-linux-gnu", "4.3"))
if (!dir.exists(user_lib)) dir.create(user_lib, recursive = TRUE, showWarnings = FALSE)
.libPaths(c(user_lib, .libPaths()))

needed <- c("tidyverse", "data.table", "cowplot")
for (p in needed) {
  if (!requireNamespace(p, quietly = TRUE)) {
    install.packages(p, repos = "https://cran.rstudio.com", lib = user_lib)
  }
}
...
#-------------------------------------------------
gff_file <- "/data/users/hfatinikun/assembly_annotation_course/annotation/EDTA/asm.bp.p_ctg.fa.mod.EDTA.raw/asm.bp.p_ctg.fa.mod.LTR.intact.raw.gff3"
cls_file <- "/data/users/hfatinikun/assembly_annotation_course/annotation/TEsorter/asm.bp.p_ctg.fa.mod.LTR.raw.fa.rexdb-plant.cls.tsv"
